...
warp 是纵向封装，管节点内部。模型用 ModelHandler / Model，工具用
ToolHandler / Tool，链式组装共用 Chain。

装饰器在每次 Run 组装时由引擎注入 per-Run 的 event.Emitter：节点内部的
事件（重试、降级）经它显式发出；warp 实例 per-Run 化，状态不跨 Run 共享。
*/
package warp
...
/*
Chain 用装饰器链包装 node：先注册的位于最外层，即调用依次经过

h1 → h2 → ... → node。每次 Run 调用。
*/
